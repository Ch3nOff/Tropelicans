...
## One-command web setup

After installing Tropelicans, users can run a single command:

```bash
tropelicans
```

That command creates `tropelicans.config.json` on first run, starts the local web chat view, and opens the browser. Users can then chat from the web view instead of manually running Python module commands.
